<div
    ...
    @endif
>
<div class="grid grid-row gap-2 justify-center">
    <div class="flex my-4 gap-6 justify-center">
        <h1 class="font-medium ">Creator :{{$record->user->name??''}}</h1>
    </div>
    <div class="flex my-4 gap-6 justify-center">
        <h2 class="mb-2 font-medium ">Task Name :{{ $record->name??'' }}</h2>
    </div>
    <div class="flex my-4 gap-6">
        <p class="text-sm ">Description :{{$record->description??''}}</p>
    </div>
    <div class="flex my-1">
        <p class="">Start  Date :{{ $record->started_at??''  }}</p>
    </div>
    <div class="flex my-1">
        <p class="">End  Date :{{ $record->end_date??''  }}</p>
    </div>

    <div class="flex my-4 gap-6">
        <a class="flex mr-4 items-center text-xs text-gray-500 gap-2" href="#">
          <span class="mr-2">
          ...
          <span>{{ $record->comment->count() }}</span>
        </a>
        <a class="flex items-center text-xs text-gray-500 gap-2" href="#">
          <span class="mr-2">
          ...
          <span>{{ $record->category->count() }}</span>
        </a>
      </div>
